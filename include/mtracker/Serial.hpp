...
struct MtrackerSerialMsgs
{
    ...
    public:
        ...
        {
            TrSetWheelsVelAndOdom() : header(headerChar), command(0x3), num_data_bytes(22), group_number(groupNumber), robot_number(robotNumber) {}
        ...
        struct RecSetWheelsVelAndOdom
        {
            ...
            float y;
            float theta;    // Orientation
            int16_t u_l;    // Control inputs (left, right)
            int16_t u_r;
            uint16_t crc;
        };
};
...
public:
  ...
  geometry_msgs::Twist getVelocity()
  {
    ...
    velMsg.angular.x = frame->w_l / 256.0f;
    velMsg.angular.y = frame->w_r / 256.0f;
  ...
  void setVelocity(float w_l, float w_r)
  {
    ...
    frame.w_l =  (int16_t) (w_l * 256.0f);
    frame.w_r = (int16_t) (w_r * 256.0f);
